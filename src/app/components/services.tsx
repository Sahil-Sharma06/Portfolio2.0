...
export default function Services() {
  const skillsRow1 = [
    ...
    "HTML", "CSS", "Sass", "jQuery", "Git", "REST APIs", "GraphQL"
  ];
  const skillsRow2 = [
    "React", "Next.js", "Node.js", "Express",
    "Tailwind CSS", "Bootstrap", "FastAPI"
  ];
  const skillsRow3 = [
    "Git", "Figma", "Firebase", "Postman", "MySQL",
    "MongoDB", "PostgreSQL"
  ];
  ...
  const getSkillIcon = (skill: string) => {
    switch (skill) {
      ...
      default:
        return <RiNextjsFill className="w-8 h-8 text-gray-500" />; // Default icon
    }
  };

  return (
    <section className="relative min-h-screen p-10 text-white bg-gradient-to-b from-gray-900 to-black">
      {/* Grid Overlay */}
      ...
      <div className="relative z-10 mx-auto max-w-7xl">
        ...
        <div className="grid grid-cols-1 gap-8 mb-16 md:grid-cols-3">
          ...
          <div className="p-6 transition transform border border-purple-600 rounded-lg bg-gradient-to-br from-gray-800 to-purple-900 hover:scale-105">
            <div className="flex items-center space-x-2">
              ...
              <h3 className="text-xl font-semibold">API Development & Integration</h3>
            </div>
            <p className="mt-4 text-gray-300">
        ...
        <div className="mt-16 space-y-8 overflow-hidden">
          {[skillsRow1, skillsRow2, skillsRow3].map((skills, rowIndex) => (
            <div key={rowIndex} className="flex space-x-8 animate-marquee">
              {/* Duplicate the skills to create a seamless scroll effect */}
              {[...skills, ...skills].map((skill, index) => (
                <div key={index} className="flex items-center p-4 space-x-3 bg-gray-800 rounded-lg min-w-[150px]">
                  {getSkillIcon(skill)}
                  <p className="text-sm font-semibold text-purple-400">{skill}</p>
                </div>
              ))}
            </div>
          ))}
